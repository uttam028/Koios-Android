...
public class WeekView extends View {
    ...
    private static final String TAG = WeekView.class.getName() + "_debug";
    ...
    protected void init() {
        ...
        mDefaultEventColor = Color.parseColor("#0d1f36");
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        float end = mHeaderTextHeight + mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * 24 + mHeaderMarginBottom;
//        canvas.drawText("00 AM", mTimeTextWidth + mHeaderColumnPadding, end + mTimeTextHeight, mTimeTextPaint);
        contentHeight = end - firstTop;
        Log.d("weekview_test", contentHeight + ", this is height");

//        this.getLayoutParams().height = (int)(contentHeight + mTimeTextHeight*2);
    }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        mWidthPerDay = getWidth() - mHeaderColumnWidth - mColumnGap * (mNumberOfVisibleDays-1) - mHeaderColumnPadding;
...
//            canvas.drawLines(hourLines, mHourSeparatorPaint);
        ...
        for (int dayNumber = leftDaysWithGaps + 1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            Log.d(TAG, "daylabel:" + dayLabel);

            if (dayLabel == null) {
                throw new IllegalStateException("A DateTimeInterpreter must not return null date");
            }
            canvas.drawText(dayLabel.substring(4, dayLabel.length()), startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
            canvas.drawText(dayLabel.substring(0, 3), startPixel + mWidthPerDay / 2, (int)(mHeaderTextHeight * 2.5) + mHeaderRowPadding, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
            startPixel += mWidthPerDay + mColumnGap;
        }
        ...
        if (mDateTimeInterpreter == null) {
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretDate(Calendar date) {
                    try {
//                        Log.d(TAG, "come for date formatting"+ date.toString());
